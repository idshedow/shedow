  ...
  if ((($_SESSION['user']) == ($edit_article['list_link_author'])) or (($_SESSION['user_role']) == 4)) {
    ...
    if (isset($_POST['submit'])) {
      ...
      if (($_SESSION['lang']) == 'en') {
        ...
        $edit = $db->prepare("UPDATE link_list SET list_link_header= :header,
          list_link_text = :text WHERE id =" . $edit_article['id']);
        $edit->bindParam(':header',$header);
        $edit->bindParam(':text',$text);
      }
      // edit only ukrainian version of article 
      else {
        ...
        $edit = $db->prepare("UPDATE link_list SET list_link_header_ua = :header,
          list_link_text_ua = :text WHERE id =" . $edit_article['id']);
        $edit->bindParam(':header',$header);
        $edit->bindParam(':text', $text);
      }
      $edit->execute();
